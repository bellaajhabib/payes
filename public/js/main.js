...
(function ($) {
    ...
    $('#CF').on('change', function () {
        ...
        if (this.value == 1) $("#NBENFANTS").prop('disabled', false);else {
            $("#NBENFANTS").prop('disabled', true);
        }
    });
    ...
    $("#IDprime").change(function () {
        ...
        $.each(montant_prime, function (index, value) {
            ...
            if (value > 1 && $("#input_montant_prime_" + value).length < 1) {
                ...
                $(".container-montant-prime").append('<div class="prime">' + '<label class="label_montant_prime">' + nom_prime[index] + ': </label>' + '<input type="text" name="montant_prime[]" class="input_montant_prime" id="input_montant_prime_' + value + '"/>' + ' <input type="hidden" name="montant_prime_exoneree[' + matches + ']" value="-1" />' + '<div class="checkbox-inline"> <input type="checkbox" name="montant_prime_exoneree[' + matches + ']" value="' + value + '">exonérée cnss </div>' + '<input type="hidden" name="prime_id[]" value="' + value + '" />' + '<a href="#" class="remove_field_montant_prime"">Supprimer</a></div>');
                ++matches;
            } else if (value == 1) {
                $(".prime").remove();
            }
    ...
    $(".container-montant-prime").on("click", ".remove_field_montant_prime", function (e) {
